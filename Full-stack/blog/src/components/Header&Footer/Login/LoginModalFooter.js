...
export default function ModalFooter({ handleClose, handleOpenSignUp, handleLogin, theme }) {
    const darkStyle = theme === 'dark' ? 'link-light' : '';
    return (
        <Modal.Footer className='d-flex flex-row-reverse border-0 pt-0'>
            ...
            <div className='ps-5 order-3 fw-normal'>Don't have an account?
                <span className={`link link-primary fst-italic ${darkStyle} fw-bold pointer ms-1`} onClick={handleOpenSignUp}>Sign Up</span>
            </div>
        </Modal.Footer>
    );
}
