...
export default function SignUpModalFooter({ handleClose, handleOpenLogin, showError }) {
    return (<>
        <div className='ms-1 d-flex'>
            <Button type="submit" disabled={showError} className={`button ${pillButtonSolid}`}>Sign Up</Button>
        ...
        <div className={`me-4 ms-3 mt-3 fw-normal ${show.xs}`}> Already have an account?
            <span className='link link-color pointer ms-1' href='#' onClick={handleOpenLogin}>Sign In</span>
        </div>
    </>);
}
