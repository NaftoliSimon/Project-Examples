...
const CustomMenu = React.forwardRef(({ children, style, className, 'aria-labelledby': labeledBy }, ref) => {
    ...
    const searchbarFilter = ( //TODO: get searchbar to filter properly
        ...
        <ul className="list-unstyled">
          {React.Children.toArray(children).filter((child) => {
            ...
            if (typeof child !== "undefined" && child.props && typeof child.props.children !== "undefined" && child.props.children !== null) {
              ...
              return (child.props.children[1]?.toLowerCase().includes(value.toLowerCase()) ||
                child.props.children[1]?.toLowerCase().replaceAll(" ", "").includes(value.toLowerCase()));
            }
            return true;
          })
            .map((child) => {
